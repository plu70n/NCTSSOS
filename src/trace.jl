...
function ptraceopt_first(tr_supp, coe, n, d; TS="block", QUIET=false, constraint="projection", solve=true)
    println("***************************NCTSSOS***************************")
    ...
    for i = 1:length(tr_supp)
        supp[i] = [ncbfind(ptsupp, length(ptsupp), tr_supp[i][j]) for j=1:length(tr_supp[i])]
    end
    if QUIET == false
...
function ptrace_SDP(supp, coe, ptsupp, wbasis, tbasis, basis, blocks, cl, blocksize; QUIET=false, constraint="projection", solve=true)
...
function constraint_reduce!(word; constraint="projection")
...
function get_ncgraph(ksupp, ptsupp, wbasis, tbasis, basis; constraint="projection")
    lb = length(wbasis)
    G = SimpleGraph(lb)
    lksupp = length(ksupp)
    for i = 1:lb, j = i+1:lb
...
function get_ncblocks(ksupp, ptsupp, wbasis, tbasis, basis; sb=[], numb=[], TS="block", QUIET=false, constraint="projection")
    if TS == false
